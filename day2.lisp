...
(defun valid-count-occurrences (password)
  ...
  (let ((occurrences (count (password-char password) (password-data password))))
    (and (>= occurrences (password-min-index password))
         (<= occurrences (password-max-index password)))))
...
(defun valid-index-occurrences (password)
  ...
  (let ((search-char (password-char password))
    ...
    (neq (char= (char data (1- (password-min-index password))) search-char)
         (char= (char data (1- (password-max-index password))) search-char))))
